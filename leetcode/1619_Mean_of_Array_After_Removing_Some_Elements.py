...
class Solution:
    def trimMean(self, arr: [int]) -> float:
        arr_len = len(arr)
        number_to_remove_each_side = int(arr_len * .05)

        arr.sort()

        final_array = arr[number_to_remove_each_side : -number_to_remove_each_side]
        
        final_len = len(final_array)

        return sum(final_array) / final_len
