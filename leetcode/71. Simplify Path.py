...
class Solution:
    def simplifyPath(self, path: str) -> str:
        ...
        for i in range(len(directories) - 1, -1, -1):
            if directories[i] == "..":
                stack.append("pop")
            elif directories[i] and directories[i] != ".." and directories[i] != ".":
                if stack:
                    stack.pop()
                    continue
                else:
                    correct_directories.append(directories[i])
        ...
        for i in range(len(correct_directories) -1, -1, -1,):
            answer += "/" + correct_directories[i]
