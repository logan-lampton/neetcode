...
class Solution:
    def maxIceCream(self, costs: [int], coins: int) -> int:
        ...
        ice_cream_bars_purchased = 0

        for ice_cream_bar_cost in sorted_costs:
            if remaining_coins >= ice_cream_bar_cost:
                ice_cream_bars_purchased += 1
                remaining_coins -= ice_cream_bar_cost
            else:
                break

        return ice_cream_bars_purchased
