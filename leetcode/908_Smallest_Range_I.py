    ...
    def smallestRangeI(self, nums: [int], k: int) -> int:
        ...
        for i in range(n):
            if nums[i] <= largest_num:
                if nums[i] + k < largest_num - k:
                    changed_nums.append(nums[i] + k)
                else:
                    changed_nums.append(largest_num - k)
            else:
                if nums[i] - k >= smallest_num + k:
                    changed_nums.append(nums[i] - k)
                else:
                    changed_nums.append(smallest_num + k)
                
        return max(changed_nums) - min(changed_nums)
