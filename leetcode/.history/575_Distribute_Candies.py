...
class Solution:
    def distributeCandies(self, candyType: [int]) -> int:
        # grab the number of candies in total
        n = len(candyType)
        # solve for number of candies that can be eaten
        daily_candies = int(n / 2)

        # see how many kinds of candies there are
        types_of_candies = len(set(candyType))

        # compare the amount she can eat to the amount of types; return the smaller amount
        return min(daily_candies, types_of_candies)
