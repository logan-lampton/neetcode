...
class Solution:
    def allCellsDistOrder(self, rows: int, cols: int, rCenter: int, cCenter: int) -> [[int]]:
        
        def distance_from_center(point):
            row, column = point
            dist_row = abs(row - rCenter)
            dist_col = abs(column - cCenter)
        ...
        return sorted(points, key=distance_from_center)
