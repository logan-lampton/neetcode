...
class Solution:
    def findPoisonedDuration(self, timeSeries: [int], duration: int) -> int:
        ...
        last_poisoned_time = timeSeries[0]

        for i in range(1, len(timeSeries)):
            if last_poisoned_time > (timeSeries[i] - duration):
                seconds_poisoned += (timeSeries[i] - last_poisoned_time)
            else:
                seconds_poisoned += duration
            last_poisoned_time = timeSeries[i]
