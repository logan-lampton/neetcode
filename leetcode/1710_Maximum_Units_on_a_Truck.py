...
class Solution:
    def maximumUnits(self, boxTypes: [[int]], truckSize: int) -> int:
        ...
        while truckSize and i < len(sorted_boxes):
            if truckSize >= sorted_boxes[i][0]:
                max_units += sorted_boxes[i][0] * sorted_boxes[i][1]
                truckSize -= sorted_boxes[i][0]
            else:
                max_units += truckSize * sorted_boxes[i][1]
                truckSize = 0
            i += 1
